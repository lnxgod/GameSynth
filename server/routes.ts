...
<body>
  ...
  app.post('/api/assets/analyze-game', async (req, res) => {
    try {
      const { code } = req.body;
      ...
      logApi("Analyzing game code for asset mapping", { codeLength: code.length });
      
      // Request OpenAI to extract mappable game objects from the code
      const prompt = `
        Analyze the following HTML5 game code and identify objects that could have images or SVG icons mapped to them.
        Find elements like game characters, items, backgrounds, UI elements, etc. that could be visually enhanced.
        ...
        Only return objects that would make sense to map visual assets to. Ignore structural elements that don't represent visual entities.
      `;
      
      const result = await makeOpenAIRequest({
        messages: [
          { role: 'system', content: 'You are a game development assistant that analyzes HTML5 game code to identify visual elements that could be enhanced with better graphics.' },
          { role: 'user', content: prompt }
        ],
      ...
      // Parse the response to extract the game objects
      const gameObjects = JSON.parse(result.choices[0].message.content);
      
      res.json(gameObjects);
    } catch (error: any) {
      console.error('Error analyzing game code:', error);
      res.status(500).json({ error: 'Failed to analyze game code: ' + error.message });
    }
  });
  
  app.post('/api/assets/generate-code-updates', async (req, res) => {
    try {
      const { gameCode, mappings } = req.body;
      
      if (!gameCode || !mappings || !Array.isArray(mappings)) {
        return res.status(400).json({ error: 'Missing game code or mappings' });
      }
      
      logApi("Generating code updates for asset mappings", { 
        codeLength: gameCode.length, 
        mappingsCount: mappings.length 
      });
      
      // Request OpenAI to generate code updates for the mappings
      const prompt = `
        I have a HTML5 game with the following code:
        \`\`\`html
        ${gameCode}
        \`\`\`
        
        I want to map the following visual assets to game objects:
        
        ${mappings.map((mapping: any) => `
        Object: ${mapping.gameObject}
        Asset Type: ${mapping.assetType}
        Asset Data: ${mapping.assetType === 'svg' ? 'SVG code provided' : 'Image URL provided'}
        Description: ${mapping.description || 'N/A'}
        `).join('\n')}
        
        Please update the code to incorporate these assets, ensuring they are properly displayed in the game.
        For image assets, use appropriate img tags or CSS background-image with the given URL.
        For SVG assets, embed them directly in the code, replacing existing elements or adding them where appropriate.
      ...
      const result = await makeOpenAIRequest({
        messages: [
          { role: 'system', content: 'You are a game development assistant specialized in updating HTML5 game code to incorporate visual assets. Your updates should be complete, functional, and maintain the original game logic while enhancing the visuals.' },
          { role: 'user', content: prompt }
        ],
        model: 'gpt-4o',  // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
        max_tokens: 4000
      ...
      const updatedCode = result.choices[0].message.content;
      
      res.json({ updatedCode });
    } catch (error: any) {
      console.error('Error generating code updates:', error);
      res.status(500).json({ error: 'Failed to generate code updates: ' + error.message });
    }
  });
